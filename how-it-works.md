...
## 1. Pi writes reports [#1-pi-writes-reports]
...
## 2. Pi wakes the plugin [#2-pi-wakes-the-plugin]
...
## 3. The store keeps one row per agent location [#3-the-store-keeps-one-row-per-agent-location]
...
## 4. The plugin reads snapshots [#4-the-plugin-reads-snapshots]
...
## 5. Stale entries are removed [#5-stale-entries-are-removed]
...
### Store garbage collection [#store-garbage-collection]
...
### Plugin lease expiry [#plugin-lease-expiry]
...
### Closed pane filtering [#closed-pane-filtering]
